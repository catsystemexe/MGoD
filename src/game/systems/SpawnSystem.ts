...
const r = (typeof def.radius === "number" && Number.isFinite(def.radius) && def.radius > 0) ? def.radius : 4;
          ...
          const camX = Number(this.world?.scrollX ?? 0);
            const camY = Number(this.world?.scrollY ?? 0);

            const spawnSpace =
              ((p as any)?.spawnSpace === "world") ? "world" : "screen";

            // store WORLD coords; if spawn is screen-relative, add camera ONCE
